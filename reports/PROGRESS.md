...
- [x] 已完成 - 需求.txt「待解决」: 产出 ssets/短视频宣传话术.csv(UTF-8 BOM),9 个角度共 51 行话术模板(开场钩子/房源硬指标/价格解析/地段与配套/参拍流程/风险提示/常见误区/紧迫感/行动号召),混合「口播句+数据占位符+固定文案」,含派生占位符{折扣率}{单价}{省额}
- [ ] 待办 - 视频脚本技能消费该 CSV: 读库按占位符填充生成每套房源口播稿(供 TTS/视频),本次未实现
- [x] 修订 - 话术库 v1.1: 删除「序号/占位字段」两列;「地段与配套」新增 POI 最近距离派生占位符(地铁/学校/医院/商场/公园),共 55 行


## 2026-08-21 (promo-image)

- [x] 新增 skill skills/promo-image: SKILL.md + scripts/compose.py
- [x] 功能: DB 读字段 → 填充 CSV 话术占位符 → 逐张房源图合成「标题+图+话术」海报 → 写回 DB
- [x] 尺寸统一: 每套房源所有海报共用 canvas(宽取原图最大宽,高由最高图+标题带+话术带推导),不硬编码
- [x] 字体: 江城律动圆.ttf(标题),极影毁片圆.ttf(正文),均取自 assets/fonts/
- [x] 实测: gpai/52946(8 张原图→6 张海报),1000x2005 统一,36.80万/43.30万/8.5折等正确填充,DB 写入 OK


## 2026-08-21 (promo-image 修订)

- [x] 修复: 不再硬截断为 6 张, 改为按实际原图数量生成全部海报
- [x] 新增双版式: 每套房源每张原图生成竖版 9:16 + 横版 16:9 各一张, 适配不同平台
- [x] 画布严格 9:16/16:9(由图片最大宽推导基准, 不硬编码像素), 标题/话术带在画布内排版
- [x] 角度分配优化: 首图=开场钩子, 末图=行动号召, 中间按逻辑序(硬指标→价格→地段→误区→风险→紧迫感)填充, 不截断不跳号
- [x] 区分海报与原图: 海报文件名 poster_{nn}_v.png/_h.png, _get_images 跳过 poster_ 前缀, 生成前清理旧海报
- [x] 实测 gpai/52946: 8 图→16 张海报(8竖1000x1778 + 8横1778x1000), 角度序正确, DB 写入 16 条(含 orientation 字段)


## 2026-08-21 (promo-image 微调)

- [x] 话术标点清理: 新增 _clean_text, 合并连续标点(,,→,、:；→:)、去除行首空占位符标点, 保留句末正常！？。
- [x] 话术与标题均居中: 话术带水平居中+自动换行; 标题带水平+垂直居中, 过长则按宽度换行(竖版窄更易爆行)
- [x] 标题自适应字号: 竖版窄宽下若换行后占高过大, 缩小标题字号(48→24)直至图片区 >= 300px 下限, 保证 9:16 内排版
- [x] 实测 gpai/52946: 竖版标题 2 行/横版 1 行, 文案无连续标点, 像素校验标题与话术均居中


## 2026-08-21 (promo-image 三轮修订)

- [x] 话术选择修复: fill_templates 仅采用「所有占位符均有值」的数据行, 否则退回无占位符固定行, 跳过残缺行(不再输出 {区域} 等未填充话术)
- [x] 字号自适应: 标题/正文随画布宽放大(竖版标题~62>正文~38; 横版标题~111>正文~68), 标题恒大于正文(下限=正文+4)
- [x] 落地需求.txt 待解决: 图片下方左下方标注投资/法律咨询信息(深圳市特资投资集团公司/0755-21677539/广东勤润律师事务所/1379246426@qq.com), 新增 CONTACT_LINES 常量, 左下角小字不与居中话术重叠
- [x] 实测 gpai/52946: 8 角度文案均完整填充或固定, 无 {占位符} 残留; 9:16/16:9 各 8 张, 联系信息位于左下角


## 2026-08-21 (promo-image 横版原图过小修复)

- [x] 横版 16:9 改为**满版**布局: 原图 cover 铺满整张 1778x1000 画布, 顶部标题带/底部话术+联系带叠加半透明白底(透明度~205), 彻底解决横版原图被压成细条的问题
- [x] 竖版保持分层布局, 但图片带由 contain 改为 cover(填满图片带、居中裁剪, 去两侧/上下留白)
- [x] 新增 _band() 半透明带绘制(在 RGB 画布上手绘 alpha 混合); _compose_poster 增加 ullbleed 参数区分两版式
- [x] 实测 gpai/52946: 横版整张画布 99%+ 为照片(顶部带因半透明仍透出照片), 竖版图片带 99% 填满; 两种版式均正常写库


## 2026-08-21 (promo-image 横版底部白带过短修复)

- [x] Bug: _band() 入参用的是裁剪 box 的**终点坐标**, 但底部白带误传了 copy_height+contact_height(=532) 当作 y1 终点, 导致白带实际只画在 y=468~532(约 64px), 话术文字落在白带下方照片上
- [x] 修正: 底部白带 y1 改为 canvas_height(=1000), 现白带完整覆盖 y=468~1000(532px), 话术与联系信息均落在半透明白带上
- [x] 实测 gpai/52946 横版: 顶部白带(0~182)与底部白带(468~1000)均正确, 底部存在约 9 万暗色文字像素, 联系信息左下角可见


## 2026-08-21 (promo-image 横版底部白带过大修复)

- [x] 横版底部原占用 ~53% 画布(话术346+联系186=532/1000), 用户反馈不应占原图一半
- [x] 横版字号调小: 标题 宽/16→宽/20(~89)、正文 宽/26→宽/34(~52); 话术带 padding 40→24; 联系带字号系数 0.5→0.45、行距 1.3→1.25、+10→+6
- [x] 竖版字号/布局保持不变; 联系带公式(共享)略减, 影响很小
- [x] 实测 gpai/52946 横版: 底部白带降至 ~295px(画布 30%), 话术与联系信息仍在白板上、联系信息左下角可见

## 2026-08-21 (阿里资产海报全量 + 话术质检)

- [x] **阿里资产海报全量**: 227 个 item 生成 226 个(1 个 1071058461504 图片未下载 file=null, 属爬虫缺口, 无法生成)
- [x] **POI 提取修复**: 阿里 POI 除 parks 外均为嵌套 dict{子类:[...]}, 旧逻辑只认平铺 list → 地铁/学校/商场/医院距离全空。改为:
  - 兼容平铺 list 与嵌套 dict(扁平化取全部)
  - 距离统一归一为**纯米数**(m/km→米, 修掉 "712m米" 问题)
  - 语义优先: 最近地铁只取 transportation.地铁; 医院优先综合医院→其他医院; 学校优先小学→中学; 商场优先购物中心; 无则退回整类
- [x] **地段与配套模板优化**: CSV 插入「地段+配套」组合行(坐落+最近地铁+最近医院)置于首行, 阿里用 POI 实距, 公拍无 POI 自然落到「坐落」行
- [x] **批量提速与断点续跑**: run_all 改为进程内循环(原先每个 item 起一个 python 进程); 已生成 script_images 的房源默认跳过(幂等), 新增 --force 强制重生成
- [x] 话术质检: 阿里 226 条 + 公拍 80 条均 0 问题(无占位符残留/无连续标点/无单位错拼/无 10 折句式)

## 2026-08-24 (话术逻辑修复 + 单调性优化 + 全量重生成)

- [x] **话术填充改版**(compose.py fill_templates): 由「取第一条全填行, 否则固定行」改为按优先级收集+按 item_id 随机:
  1) 全填行(所有占位符可填) → 随机取一
  2) 宽松行(过半可填) → 未填占位符所在**整段按标点切段删除**, 避免"最近医院米"残句 → 随机取一
  3) 固定兜底行 → 随机取一
- [x] CSV 新增「宽松填充」列: 默认是, `否` 标记断言行(如"中间楼层", 楼层缺一即不成立); 无折扣(起拍=评估价)自动跳过「10折/立省≤0」句式
- [x] **逻辑问题修复**: ① 坐落/标题括号泄漏(【】〔〕含未闭合截断); ② "调查情况表"等字段名残句黑名单; ③ "位于/坐落于"前缀; ④ 空格("） 戊类"); ⑤ 句尾逗号族; ⑥ "一拍起拍"事实性误导→"若本轮流拍…可能更低"; ⑦ "立省0.00万"拦截
- [x] **全量重生成**: 公拍 80/80 + 阿里 226/226(1 个无图), 306 条话术 QA 0 问题
- [x] **单调性**: 紧迫感 1→306 种句式(100% 唯一), 行动号召 1→5 种, 风险提示 top行占比 176→64, 房源硬指标 top行占比 251→91, 常见误区 top行占比 67→24
- [x] 新增 `tests/test_promo_image.py`(6 个契约测试: 角度齐全/无占位符/宽松整段删除/无折扣跳过/seed 确定性/句尾标点)

## 2026-08-24 (查重/不合理话术巡检 + 数据清洗加固)

- [x] **全量巡检 306 条**: 整篇重复 **0** 组; 跨角度折扣率矛盾(开场 vs 价格解析) **0**; 角度缺失/多余 **0**; 残留占位符/连续标点/单位错拼/0价格/10折 **0**
- [x] 修复 gpai/53063 类数据错误: 建筑面积取到源数据垃圾值 `150921.40㎡`(真实合计 208.59) → **建筑面积候选键优先 `建筑面积_合计`**
- [x] **面积上限**: 建筑面积 >100万㎡ 视为垃圾值(车位/多套粘连)置空; **单价 <0.005万/㎡ 置空**(防"单价0.00万/㎡")
- [x] **标题加固**: 剥 ≥6位超大㎡残段; 坐落剥尾部",XX区"式截断残段
- [x] **模板**: 紧迫感「到点预告」由 `{小区名称}{建筑面积}㎡` 改为 `{小区名称}共{建筑面积}㎡`, 避免"车位207208.59㎡"拼接歧义
- [x] 全量重生成: 公拍 80 + 阿里 226 均已应用新清洗; 测试增至 7 个契约用例
...
- [x] **方案 Y 落地**: 话术生成从 promo-image 抽离为独立技能 `skills/script-writer/`
  ...
  - 真实调用验证: gpai/52946 润色成功(llm=on), 数字全守住、句式多样、72s/条
- [x] **promo-image 瘦身**: compose.py 删除话术逻辑, 改为读 DB `data.script` 解析 8 角度 → 合成海报 → 写 `data.script_images`; 无话术房源提示先跑 script-writer
- [x] **agent/ 智能体核心层**: `agent/model.py` = DashScope(OpenAI 兼容) 统一 LLM 客户端(重试/限流/空key抛错), 供各技能/任务复用; `config.py` 读 `AliBaseURL/AliAPIKey/AliLLM`
- [x] 测试: 话术契约测试迁至 `tests/test_script_writer.py`(10 例, 含 LLM 校验逻辑), promo-image 改解析/角度分配测试(4 例); 全量 109 通过
- [x] AGENTS.md 更新: `agent/` 目录约定 + LLM 基座分层 + 话术/海报职责分离

## 2026-08-24 (LLM 增强调试修复 — 从"全失败"到"3/3 成功")

- [x] 首轮 3 套 ali 试跑 0 成功, 诊断为三类问题并逐一修复:
  1. **超时**: qwen3.7-plus 单条约 70s, 原 timeout=60 太短 → 默认提到 180s(`AliTimeout` 可配)
  2. **校验器误杀**: 事实字段为合并串"5层，共18层", `_allowed_nums` 整体 float 失败导致 5/18 未进允许集 → 改为正则提取字段内全部数字; 同轮发现"学校1000米"等其实来自事实字段, 非幻觉
  3. **模型输出残缺**: 偶发只回 5 个角度/JSON 截断/把评估价写成起拍 → 精简 prompt(素材库只给子主题+备注, 不再整段模板)+ **失败语义重试 3 次**(附失败原因提示)
- [x] 修复后 3/3 套成功(1064719406791/1064721326148/1064730494048), 数字全守住、真实楼层/POI 补全、8 角度句式多样
- [x] 校验器升级: `_find_issue` 输出具体拒绝原因(缺哪些角度/哪个数字越界), 便于后续排障

## 2026-08-24 (备注列语义澄清 + 源准入规则化)

- [x] **修正 CSV 列错位**: 此前加「宽松填充」列时 11 行旧备注串位到该列, 已全部归位; 删除过时备注(低价捡漏的"区域缺失置空", 模板已无 {区域})
- [x] **结论: 备注≠需要 LLM 检查**。逐条核对 12 条备注: 派生值(折扣率/单价/省额)由代码计算、最近*距离行字段为空天然不可选、断言行由宽松填充=否强制 —— 规则已兜住; 唯一真漏洞是「实测配套」固定行(无占位符, gpai/无 POI 房源也可能选中并谎称"阿里源实测")
- [x] **源准入规则化**(替代 LLM 检查): `_row_allowed` — 备注含「仅 ali」的行要求 source=ali 且 POI 距离至少一项非空; fill_templates 增加 source 参数
- [x] 移除上一轮的"单薄门槛"(angle_kinds/thin_angle_count/--llm-threshold), 设计回归简单: 默认纯规则随机, --llm 为显式润色开关
- [x] fill_templates_meta 返回选中行元数据(子主题/备注)备用; 测试 111 通过(新增源准入契约)

## 2026-08-24 (话术库离线补充 56→91 行 + 实测配套过度宣称修复)

- [x] **实测配套行修复**(用户指出"有 POI ≠ 步行范围"): 原固定行宣称"地铁/学校/医院/公园均在步行范围内"属过度宣称(医院可能 2400 米), 改为数据行用真实距离说话("最近地铁{距离}米、最近公园{距离}米，配套远近自己判断"), 缺失段自动删; 源准入门槛保留(仅 ali 且至少一项 POI)
- [x] **话术库补充 56→91 行**: 开场钩子 6→10, 房源硬指标 6→11, 价格解析 6→11, 地段与配套 11→15, 常见误区 6→10, 风险提示 6→10, 紧迫感 4→8, 行动号召 5→10; 新增行全部走既有占位符/宽松填充/源准入机制, 无新逻辑
- [x] 4 套房源(2 gpai + 2 ali)端到端验证: 新行自然入选、无占位符残留、无连续标点; 测试 111 通过
- [ ] 待办: 存量 306 套 DB 话术仍是旧库所选, 需 --force 重生成才能用上新行(话术+海报约 50 分钟); CSV 中「参拍流程」角度 6 行不在 ANGLE_ORDER 从未启用, 待定用途

## 2026-08-24 (步行范围规则判定 + 话术库数量确认 56→92)

- [x] **「步行范围内」改为规则判定**(用户指出"远近可以规则判断, 有 POI≠步行范围"): `_walking_ok` 要求 地铁/学校/医院/公园 距离**全部存在且≤1200米**才放行宣称行(实测配套); 备注作为声明式条件由 `_row_allowed` 解析(仅 ali / 步行范围), 医院2400米、缺类别、gpai/无POI 一律拒
- [x] 保留数据行「配套实测距离」(真实距离, "远近自己判断")作为无门槛兜底
- [x] **话术库数量确认(56→92, 全部增加, 之前"→10"被误读为"→1")**: 开场钩子6→10, 房源硬指标6→11, 价格解析6→11, 地段与配套11→16, 常见误区6→10, 风险提示6→10, 紧迫感4→8, 行动号召5→10, 参拍流程6(未启用)
- [x] 测试 113 通过(新增步行门槛单元+端到端契约); 真实数据验证: 医院2400米房源不出现"均在步行范围内"

## 2026-08-24 (视频拼接技能 video-compose + LLM 需求量分析)

- [x] **LLM 需求量化**: 307 条话术分析 — 按"≥3 个数据角度仅固定兜底(单薄)"口径, **83 条建议 LLM 润色**(ali 79 + gpai 4), 其余 224 条规则稿已充实; "选中带备注行"口径过宽(214, 杠杆误区几乎每条命中), 不作触发
- [x] **video-compose 技能**(TTS 暂不参与): 读 promo-image 海报 poster_*_v/_h.png → imageio-ffmpeg 捆绑 ffmpeg concat 拼接 → assets/<source>/<item_id>/video/<id>_{v,h}.mp4 → 写 DB data.video{vertical,horizontal}
- [x] 验证: gpai/52946(8张×4s=32s h264) + ali/1064719406791(5张×4s=20s h264), 解码 0 错误; CLI --all/--force/--duration/--fps/幂等跳过; 测试 116 通过
- [x] 依赖: 新增 imageio-ffmpeg(自带 ffmpeg-win-x86_64-v7.1, 无需系统安装)

## 2026-08-24 (全量视频完成: 306 套, 5 套 LLM 润色, TTS 未上)

- [x] **5 套 LLM 润色**(ali: 1071777750546/1070936371704/1072804489406/1071777874524/1070936491683): 话术(LLM)→海报→视频 全链完成, 无回退, 数字全守住
- [x] **全量视频拼接 306 套**(gpai 80 + ali 226, workers=3, 4s/张), 0 缺失; DB data.video 全部落库
- [x] 批量中修 3 个真问题:
  1. **workers=8 → WinError 1455 页面文件爆**(8 个 ffmpeg 并行超内存) → 降为 3, 续跑
  2. **奇数尺寸海报 → libx264+yuv420p "Invalid argument"** → 每路输入 scale 偶数化
  3. **4096×7282 超大海报 → 超 libx264 宏块上限编码失败** → 长边限 1920(不放大, 顺带提速)
- [x] video-compose 增加 --workers 并发; 测试 116 通过

## 2026-08-24 (资产布局重构 + 视频配音合成 + TTS 准备)

- [x] **资产布局重构**: assets/<src>/<id>/ 改为 imgs(原始图)/posters(海报)/videos(视频)/voice(TTS); compose.py 海报→posters/, make_video 读posters/写videos/, 已迁移 2884 海报 + 612 视频, DB data.video 路径同步修 306 条
- [x] **视频+配音(mux_voice.py)**: 每张海报段时长=对应角度音频时长, 音轨逐段 concat, 声画对齐; 解决"视频固定时长 vs 音频长短不一"; 输出 <id>_{v,h}_voiced.mp4, DB data.video_voiced; 实测 gpai/52946 (66s, 含音轨)
- [x] **TTS 准备(voice-tts skill, edge-tts 免费无key)**: 逐角度 mp3 → voice/ + 整篇(只连正文不读【角度】标记); 失败重试退避防限流; gpai/52946 8/8 成功, ali 后台 7/8
- [x] 批量中修: 时长解析字节安全(Windows GBK), edge-tts 限流退避+1s 间隔
- [x] AGENTS.md 资产目录约定更新; 测试 116 通过

## 2026-08-24 (海报标题去括号)

- [x] promo-image 海报标题显示前 `_clean_title`: 剥离 拍次标签(【一拍】等) + 括号注释(【…】[…]〔…〕, 含未闭合截断), 与 script-writer 清洗口径一致
- [x] 扫描: 91 套标题含括号(多为 gpai 的【一拍】前缀 + 注释); 仅重生成这 91 套的海报+静音视频(并发4, 13分钟), 未动其余 215 套(标题无括号不受影响)
- [x] 测试 +1(标题清洗契约), 117 通过

## 2026-08-26 (过期清理 + 少图保底4海报)

- [x] scripts/purge_expired.py: 删过期(start_time<执行时刻now, 可反复跑) DB行+assets目录同步; 本次清 157 套(gpai17+ali140), 余 151
- [x] promo-image 少图保底: _expand_images 1图→[A,A,A,A] / 2图→[A,A,B,B](用户指定顺序), 各配不同角度话术(钩子/硬指标/价格解析/号召); ≥3图不变
